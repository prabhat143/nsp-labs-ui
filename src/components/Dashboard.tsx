    ...
    <div className="space-y-4 sm:space-y-6 lg:space-y-8">
      ...
      {loading && (
        <div className="bg-white rounded-2xl shadow-lg p-6 sm:p-8 lg:p-12 text-center">
          <FlaskConical className="h-8 w-8 sm:h-12 sm:w-12 lg:h-16 lg:w-16 text-gray-300 mx-auto mb-4 animate-pulse" />
          <h3 className="text-base sm:text-lg lg:text-xl font-semibold text-gray-900 mb-2">Loading Dashboard...</h3>
      ...
      {error && (
        <div className="bg-white rounded-2xl shadow-lg p-6 sm:p-8 lg:p-12 text-center">
          <AlertTriangle className="h-8 w-8 sm:h-12 sm:w-12 lg:h-16 lg:w-16 text-red-300 mx-auto mb-4" />
          <h3 className="text-base sm:text-lg lg:text-xl font-semibold text-gray-900 mb-2">Error Loading Dashboard</h3>
          <p className="text-red-600 mb-6 text-sm lg:text-base">{error}</p>
          <button
            onClick={() => window.location.reload()}
            className="bg-gradient-to-r from-cyan-500 to-blue-500 text-white px-4 sm:px-6 py-2 sm:py-3 rounded-lg hover:from-cyan-600 hover:to-blue-600 transition-colors text-sm sm:text-base"
          >
            Try Again
          </button>
        </div>
      ...
      {!loading && !error && (
        <>
          {/* Welcome Header */}
          <div className="bg-gradient-to-r from-cyan-500 to-blue-500 rounded-2xl p-4 sm:p-6 lg:p-8 text-white">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 sm:gap-4 lg:gap-6">
            {stats.map((stat, index) => (
              <div 
                ...
                className={`bg-white rounded-xl p-3 sm:p-4 lg:p-6 shadow-lg transition-all duration-200 ${
                  stat.clickable 
              ...
              >
                <div className="flex flex-col sm:flex-row sm:items-center">
                  <div className={`p-2 lg:p-3 rounded-lg ${stat.bgColor} mb-2 sm:mb-0 sm:mr-3 lg:mr-4 self-center sm:self-auto`}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 lg:gap-8">
            {dashboardCards.map((card, index) => (
              <div
                ...
                className="bg-white rounded-xl p-4 sm:p-6 lg:p-8 shadow-lg hover:shadow-xl transition-all duration-200 cursor-pointer group"
              >
                <div className="flex items-start space-x-3 sm:space-x-4">
                  <div className={`p-3 lg:p-4 rounded-xl ${card.color} ${card.hoverColor} transition-colors group-hover:scale-110 transform duration-200`}>
                    <card.icon className="h-5 w-5 sm:h-6 sm:w-6 lg:h-8 lg:w-8 text-white" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <h3 className="text-base sm:text-lg lg:text-xl font-bold text-gray-900 mb-2 group-hover:text-cyan-600 transition-colors">
                      {card.title}
                    </h3>
          ...
          {samples.length > 0 && (
            <div className="bg-white rounded-xl p-4 sm:p-6 lg:p-8 shadow-lg">
              <h2 className="text-lg sm:text-xl lg:text-2xl font-bold text-gray-900 mb-4 sm:mb-6">Recent Activity</h2>
              <div className="space-y-3 sm:space-y-4">
                {samples
                  .sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime())
                  .slice(0, 3)
                  .map((sample) => (
                  <div key={sample.id} className="flex flex-col sm:flex-row sm:items-center sm:justify-between p-3 sm:p-4 bg-gray-50 rounded-lg space-y-2 sm:space-y-0">
                    <div className="flex items-center space-x-3 sm:space-x-4">
                      ...
                      <div className="min-w-0 flex-1">
                        <p className="font-medium text-gray-900 truncate text-sm sm:text-base">{sample.samplerLocation}</p>
                        <p className="text-xs sm:text-sm text-gray-500">{sample.shrimpCategory}</p>
