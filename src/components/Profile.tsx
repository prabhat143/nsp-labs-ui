...
import { User, Mail, Building, Phone, MapPin, Save, Edit,Users,Plus,Trash2 } from 'lucide-react';
...
const Profile: React.FC = () => {
  ...
  const handleUpdateCustomer = (e: React.FormEvent) => {
    ...
    const updatedCustomers = formData.customers.map(customer => 
      customer.id === editingCustomerId ? { ...customer, ...newCustomer } : customer
    );
  ...
  const handleDeleteCustomer = (customerId: string) => {
    const updatedCustomers = formData.customers.filter(customer => customer.id !== customerId);
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-indigo-100">Manage your customer information</p>
              </div>
            </div>
            <button
        ...
        <div className="p-8">
          ...
          {(showAddCustomer || editingCustomerId) && (
            <div className="mb-8 p-6 border border-indigo-100 bg-indigo-50 rounded-lg">
              ...
              <form onSubmit={editingCustomerId ? handleUpdateCustomer : handleAddCustomer} className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {/* Customer Name */}
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">
                ...
                <div className="flex justify-end space-x-4">
                  <button
                    type="button"
                    onClick={editingCustomerId ? handleCancelEditCustomer : () => setShowAddCustomer(false)}
                    className="px-6 py-3 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 transition-colors"
                  >
                    Cancel
                  </button>
          ...
          {/* Customer List */}
          <div>
            <h3 className="text-lg font-semibold text-gray-900 mb-4">Your Customers</h3>
                  ...
                  <div key={customer.id} className="py-4">
                    <div className="flex flex-col md:flex-row md:justify-between md:items-center">
                      <div className="mb-2 md:mb-0">
                        <h4 className="text-md font-medium text-gray-900">{customer.name}</h4>
